...
if [ ! -z "$(df | grep mmcblk0)" ]; then
  ...
  mounted or you're running this tool from MixOS \
  that is mounted on the internal emmc storage.  \
  Please make sure to run this tool from a sd card based \
  boot of MixOS and make sure the internal storage is not \
...
from your device and replace it with MixOS.  You've been warned!  Type OK in the next \
screen to proceed."
...
dialog --clear
(pv -n "$image" | sudo dd of=/dev/mmcblk0 bs=1M status=none) 2>&1 | dialog --gauge "Writing the MixOS \
...
sudo umount /mnt/usbdrive/
dialog --clear
msgbox "Done.  Please reboot without a SD card in slot 1 to boot into MixOS from emmc.  If you'd like to \
