...
/*
 * The loader's resting panel brightness, in percent. HALF, not full.
 *
 * VBAT is VSYS on this board -- there is no power-path FET, see the note at
 * mt6592_pmic.c:1023 -- so the panel and the backlight draw from the same node
 * the charger feeds, and every milliamp the backlight takes is a milliamp that
 * does not reach the cell. Against a 450 mA charge step a full-brightness panel
 * is a large fraction of the budget. Half is legible on this display.
 *
 * DEFINED HERE BECAUSE TWO FILES NEED IT. mvii_lk_main.c and panel_bringup.c each
 * had their own `#ifndef' copy, which is a duplicate that cannot be caught: the
 * bring-up copy lights the splash and the main copy sets the park, so a change to
 * one and not the other shows up only as a brightness step on the panel between
 * the logo and the gauge. One definition, one brightness, no step.
 *
 * Full brightness still exists in the loader, at exactly one point: the handoff,
 * where the OS is about to own the panel and the charge budget stops being the
 * loader's problem. See MVII_MT6592_LK_HANDOFF_BACKLIGHT_PCT in mvii_lk_main.c.
 */
#ifndef MVII_MT6592_LK_BACKLIGHT_PCT
#define MVII_MT6592_LK_BACKLIGHT_PCT 50u
#endif
