...
enum {
    ...
    /*
     * USB DRVVBUS — the 5 V the OTG port can source, and it is a plain GPIO like
     * the LEDs above, NOT the MT6322 boost this tree assumed for a long time.
     *
     * Read out of the stock Android kernel, not guessed. mt_usb_set_vbus() sits at
     * VA 0xc052e938 in Reference/J36-ULTRA's boot image (gzip member at file offset
     * 0x4934, load base 0xC0008000) and was located through the __func__ pointer its
     * own printk loads, which also gives its name and that it is line 60 of MTK's
     * musb glue. On the `on' path it is four instructions and they are:
     *
     ...
     *
     * 0x80000000 is MTK's "pin came from the cust header" marker, the same one the
     * keypad block below documents, stripped by the wrapper before it bounds-checks
     * the pad against 0xa8. So the pad is 15 and the drive is active high.
     *
     * Neither callee has a symbol in that image; both were identified structurally,
     * from the GPIO ops table at 0xc0b707dc rather than from a name. Slot 0x3c
     * divides the pad by five (by 0xcccccccd) and rewrites a 3-bit field at
     * base+0x600, which is MODE. Slot 0x30 writes the SET/RST alias of base+0x400,
     * which is DOUT. Both are offsets mt6592_led.c already drives on this board, so
     * this is the same map, not a second theory about it.
     *
     * The literal 0x8000000f is loaded at exactly six instruction sites in the whole
     * 12 MB image, all inside mt_usb_set_vbus and the pad-config routine beside it
     * at 0xc052e9c4. Nothing else on this board wants pad 15.
     *
     * FIT A CELL BEFORE DRIVING IT. The 5 V is a boost off VBAT, and VBAT on this
     * PMIC is the system node -- the same rail the AFE audio driver's class-D amp pulls
     * under the undervoltage lockout on a cell-less board. A bus-powered device on
     * this port is that same class of load.
     */
    MT6592_J36_USB_DRVVBUS_GPIO = 15u,
    MT6592_J36_USB_DRVVBUS_ACTIVE_LOW = 0u,
     ...
     *     gpio_value_is_pressed in the OS keypad driver). Pad 93 is in that list.
     ...
     * that does not mux them. The OS keypad driver's kpd_pads_apply() does, and only for a pad
     ...
     * pull-up can fight, and no register makes that resistor bigger. The OS keypad driver reads it
     ...
     *
     * These pads are read by an active scan now (key_wires_scan() in the OS keypad driver),
     ...
     *    the OS keypad driver (link map: MVIILK.elf.dir is console=1 keys=0) so no pull-up
     ...
     *
     * Orientation rule: the DRIVE pad is never a keypad pad. The OS keypad driver pulses it
     ...
     *
     * Kept as UNMAPPED names rather than deleted, because the OS keypad driver derives g_key_wires[]
     ...
     * and the OS keypad driver use: KPD_MEM1 bit 0 is matrix bit 0, KPD_MEM2 bit 0 is
