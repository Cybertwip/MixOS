...
then
  height="20"
  width="60"
elif test ! -z "$(grep -E 'RGB20PRO|MINILOONG' /home/virtua/.config/.DEVICE | tr -d '\0')"
then
  height="20"
  width="70"
fi
...
DeleteConnect() {
  ...
  if [[ -z $cur_ap ]]; then
    cur_ap=`sudo nmcli -t -f name,device connection show --active | grep wlan0 | cut -d\: -f1`
  fi
...
ConnectExisting() {
  ...
  sudo nmcli con down "$cur_ap" >> /dev/null
  sleep 1
...
NetworkInfo() {
  ...
  if [[ -z $currentssid ]]; then
    currentssid=`sudo nmcli -t -f name,device connection show --active | grep wlan0 | cut -d\: -f1`
  fi
...
MainMenu() {
  ...
  if [[ "$(tr -d '\0' < /proc/device-tree/compatible)" == *"rk3566"* ]]; then
    if [[ ! -z $(rfkill -n -o TYPE,SOFT | grep wlan | grep -w unblocked) ]]; then
	  ...
	  if [[ -z $cur_ap ]]; then
	    cur_ap="Currently connected to `sudo nmcli -t -f name,device connection show --active | grep wlan0 | cut -d\: -f1`"
	  fi
    else
      local Wifi_Stat="Off"
	  local Wifi_MStat="On"
